...
- **Dropping required structure**: PR template sections and the `> This is a bot response.` preamble are content or compliance requirements, not boilerplate. Keep them and write them tight. The two-paragraph commit body is a default rather than a hard rule — see [shared-rules/git-workflow.md](shared-rules/git-workflow.md) for when a shorter body is correct.
